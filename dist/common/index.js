...
exports.root_attribute_reference = void 0;
...
function _validate_attribute(attribute_name, value, expanded_schema, exact, _handle_error) {
    ...
    switch (expanded_schema.primitive) {
        case 'enum': {
            ...
            if (!possible_values.has(typeof value)) {
                _handle_error(attribute_name, `Attribute '${attribute_name}' has an invalid type. Type should be` +
                    ` one of the following ['${Array.from(possible_values)}]. Type given '${typeof value}'`);
            }
            break;
        }
        case 'unknown': {
    ...
    if (options && !options.includes(value)) {
        _handle_error(attribute_name, `Invalid attribute '${attribute_name}'. The only possible values` +
    ...
    if (expanded_schema.primitive === 'string' &&
        ...
        value === '') {
        _handle_error(attribute_name, `Missing required attribute '${attribute_name}'. The string cannot be` +
            ` empty`);
    }
    ...
    if (expanded_schema.primitive === 'string' && typeof value === 'string') {
        if (expanded_schema.minLength !== undefined &&
            value.length < expanded_schema.minLength) {
            _handle_error(attribute_name, `Attribute '${attribute_name}' must have a minimum length of ${expanded_schema.minLength}, but has length ${value.length}`);
        }
        if (expanded_schema.maxLength !== undefined &&
            value.length > expanded_schema.maxLength) {
            _handle_error(attribute_name, `Attribute '${attribute_name}' must have a maximum length of ${expanded_schema.maxLength}, but has length ${value.length}`);
        }
    }
    ...
    if (expanded_schema.properties) {
        ...
        for (const [k, v] of Object.entries(value_record)) {
            ...
            if (!(k in expanded_schema.properties)) {
                if (exact) {
                    _handle_error(k, `No additional attributes are permitted. Attribute '${k}' in not` +
                        ` in schema`);
                }
                else {
