...
type _ExpandedSchemaToTypeRequired<S extends ExpandedSchema> = S extends {
    primitive: 'object';
    properties: infer P;
} ? P extends Properties ? _PropertiesToType<P> : Record<string, any> : S extends {
    primitive: 'array';
    item: infer I;
} ? I extends Schema ? Array<SchemaType<I>> : any[] : S extends {
    primitive: infer Prim;
    values: infer V;
} ? V extends ReadonlyArray<infer Item> ? Item : Prim extends Primitive ? _PrimitiveToType<Prim> : never : S extends {
    primitive: infer Prim;
} ? Prim extends Primitive ? _PrimitiveToType<Prim> : never : never;
type _PropertiesToType<P extends Properties> = {
    [K in keyof P as P[K] extends {
        optional: true;
    } ? never : P[K] extends ExpandedSchema ? P[K]['optional'] extends true ? never : K : K]: SchemaType<P[K]>;
} & {
    [K in keyof P as P[K] extends {
        optional: true;
    } ? K : P[K] extends ExpandedSchema ? P[K]['optional'] extends true ? K : never : never]?: SchemaType<P[K]>;
};
